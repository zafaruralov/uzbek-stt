...
class UzbekSTTService:
    ...
    def load_audio(self, audio_path):
        ...
        # PyAV bilan barcha formatlarni to'liq yuklash (MP3, WAV, FLAC, OGG, ...)
        ...
        if not frames:
            raise RuntimeError(f"Audio fayldan hech qanday ma'lumot o'qib bo'lmadi: {audio_path}")

        # Birlashtirish: shape = (channels, samples)
        audio = np.concatenate(frames, axis=1)
        logger.info(f"PyAV bilan yuklandi (sample rate: {sr}Hz, channels: {audio.shape[0]})")
        ...
        # float32 ga o'tkazish (agar int formatda bo'lsa)
        ...
        logger.info(f"Audio yuklandi: {len(audio)} samples, {len(audio)/self.sample_rate:.2f}s")

        return audio

    def transcribe_chunk(self, audio_chunk, chunk_duration=None):
        ...
        # Qisqa chunklarda hallucination oldini olish
        generate_kwargs = {
            "no_repeat_ngram_size": 3,
        }
        if chunk_duration is not None:
            # Taxminan 1 soniyaga ~12-15 token, Whisper limiti: 448 - 4 (special tokens) = 444
            max_tokens = min(int(chunk_duration * 15) + 10, 444)
    ...
    def transcribe_chunk_with_timestamps(self, audio_chunk, chunk_duration=None):
        """Bitta chunk'ni timestamp'lar bilan transcribe qilish.
        Model timestamp tokenlarni qo'llab-quvvatlamaydi, shuning uchun
        matnni gaplarga bo'lib, vaqtni proporsional taqsimlaymiz."""
        text = self.transcribe_chunk(audio_chunk, chunk_duration=chunk_duration)

        if not text or not text.strip():
            return text, []

        duration = chunk_duration or (len(audio_chunk) / self.sample_rate)
        sentences = self._split_into_sentences(text)

        if not sentences:
            return text, [{"start": 0.0, "end": duration, "text": text}]
    ...
    def transcribe_with_timestamps(self, audio_path):
        ...
        try:
            ...
            if len(audio_array) <= chunk_length:
                logger.info("Qisqa audio — timestamp bilan transcribe...")
                text, segments = self.transcribe_chunk_with_timestamps(
                    audio_array, chunk_duration=duration
                )
            ...
            # Uzun audio — chunklarga bo'lib
            logger.info("Uzun audio — chunklarga bo'lib timestamp bilan transcribe...")
            overlap_length = 5 * self.sample_rate
            ...
            while start < len(audio_array):
                ...
                chunk_offset = start / self.sample_rate
                logger.info(
                    f"Chunk {chunk_num}: {chunk_offset:.1f}s - {end / self.sample_rate:.1f}s ({chunk_duration:.1f}s)")

                text, segments = self.transcribe_chunk_with_timestamps(
                    chunk, chunk_duration=chunk_duration
                )
            ...
            logger.info(f"Transcription tugadi! {chunk_num} ta chunk, {len(all_segments)} segment")
            ...
            logger.error(f"Xatolik (timestamps): {str(e)}")
            import traceback
    ...
    def transcribe(self, audio_path):
        ...
        try:
            ...
            if len(audio_array) <= chunk_length:
                ...
                text = self.transcribe_chunk(audio_array, chunk_duration=duration)
            ...
            while start < len(audio_array):
                ...
                # Transcribe chunk
                text = self.transcribe_chunk(chunk, chunk_duration=chunk_duration)
                transcriptions.append(text)
